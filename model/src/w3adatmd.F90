...
MODULE W3ADATMD
  ...
  TYPE WADAT
    ...
    !
    ! To be used under switch MFIT. XSMH(1,1) and XXSMH(1,1) will always exist
    REAL, POINTER         ::  XSMH(:,:)
    REAL, POINTER         ::  XXSMH(:,:)
    !    
    REAL, POINTER         ::  USERO(:,:)
  ...
  REAL, POINTER           :: U_S(:), V_S(:), ZK_S(:)
#endif
  ! Declare USVP(:,:) applied with W3_STVP
  REAL, POINTER           :: USVP(:,:)
  !/
CONTAINS
  !/ ------------------------------------------------------------------- /
...
!
    ...
    ! To be used under switch MFIT. WADATS(IMOD)%XSMH(1,1) will always exist
    ALLOCATE ( WADATS(IMOD)%XSMH(NSEALM,NOXSM), STAT=ISTAT )
    CHECK_ALLOC_STATUS ( ISTAT )
    !
    WADATS(IMOD)%XSMH  = UNDEF
    !
    !
...
9003 FORMAT (' TEST W3DIMA : DIMENSIONS STORED')
  ...
  SUBROUTINE W3XDMA  ( IMOD, NDSE, NDST, OUTFLAGS )
    ...
    !
    IF ( NZO .GT. 0 ) THEN
      ! Similar to: IF ( OUTFLAGS( 6, 14) ) THEN
...
#ifdef W3_MFIT
    IF (  OUTFLAGS(10, NOEXTR+1) ) THEN
      ALLOCATE ( WADATS(IMOD)%XXSMH(NXXX,NOXSM), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    ELSE
#endif
      ! XXSMH(1,1) to be used under switxh W3_MFIT shall always exist
      ALLOCATE ( WADATS(IMOD)%XXSMH(1,1), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
#ifdef W3_MFIT
    END IF 
#endif
      WADATS(IMOD)%XXSMH   = UNDEF
    !
    DO I=1, NOEXTR
